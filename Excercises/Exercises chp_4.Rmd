...
We are asked to use the equivalence principle to determine the fair premium $\pi$, we start of by describing the policy functions:
...
Since reactivation is not allowed, we can actually solve the transition-probabilites directly. We first start with the survival probability: 
...
\newpage
Now from the equivalence principle we should have that the premium $\pi$ should be decided so that $V_{*}^{+}(0,A) = 0$, this leaves us with: 
...
We then use the equivalence principle, telling us that the fair premium $\pi$ should be chosen so that $V_{*}^{+}(0,A) = 0$: 
...
Now as always, the equivalence principle tells us to chose $\pi$ such that $V_{*}^{+}(0,A) = 0$: 
$$\begin{aligned}
\pi &= 
\frac{
